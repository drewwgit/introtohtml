...
function calculateTotalInventory() {
  // const sumofInventory = items.reduce ((accumulator , items) => {
  //   return accumulator += items.inventory;
  // })
...
function getItemPriceByName(name) {
  const pricebyName = items.find((i) => i.name === name);
  return pricebyName.price;
}
...
function filterItemsByCategoryId(categoryId) {
  const categoryMatch = items.filter((i) => i.categoryId === categoryId);
...
function calculateTotalCartPrice() {
  const itemValues = cart.map(id) => findItemByID(+id).price;
  const totalCartValue = itemValues.reduce((currentTotal, itemValue) => {
    return itemValue + currentTotal; 
  }, 0);
